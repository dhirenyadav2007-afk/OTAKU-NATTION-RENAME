        ...
        try:
            ...
            try:
                ...
                await message.reply_text(
                    f"<b><i>! Eʀʀᴏʀ, Cᴏɴᴛᴀᴄᴛ ᴅᴇᴠᴇʟᴏᴘᴇʀ ᴛᴏ sᴏʟᴠᴇ ᴛʜᴇ ɪssᴜᴇs @Akuma_Rei_Kami</i></b>\n"
                ...
                try:
                    ...
                    if mode == "on" and not data.username:
                        invite = await client.create_chat_invite_link(
                            ...
                            expire_date= (
                                datetime.now(timezone.utc) + timedelta(seconds=FSUB_LINK_EXPIRY)
                                if FSUB_LINK_EXPIRY and FSUB_LINK_EXPIRY > 0
                                else None
                            )
                        )
        ...
        try:
            buttons.append([
                InlineKeyboardButton(
                    text='• Jᴏɪɴᴇᴅ •',
                    url="https://t.me/ANIME_uploader_ON_bot?start=FLINK_480p_93ccb8880f1a"
                )
            ])
        except IndexError:
            pass
...
async def show_start_message(client, message: Message):
    ...
    await message.reply_sticker("CAACAgUAAxkBAAKgmml4sN0hFhwsE-uTxERjVP5Ywdu0AALzGwAC7u0YVYRuV5msoZOEHgQ")
    ...
    buttons = InlineKeyboardMarkup([
        ...
        [
            ...
            InlineKeyboardButton('sᴜᴘᴘᴏʀᴛ •', url='https://t.me/BotifyX_support')
        ],
        [
            InlineKeyboardButton('• ᴀʙᴏᴜᴛ', callback_data='about'),
            InlineKeyboardButton('Dᴇᴠᴇʟᴏᴘᴇʀ•', url='https://t.me/ITSANIMEN')
        ]
    ])
...
async def send_verification_message(client, message: Message):
    ...
    if not shortlink:
        await message.reply_text(
            "Eʀʀᴏʀ ɢᴇɴᴇʀᴀᴛɪɴɢ sʜᴏʀᴛʟɪɴᴋ. Pʟᴇᴀsᴇ ᴛʀʏ ᴀɢᴀɪɴ ʟᴀᴛᴇʀ ᴏʀ ᴄᴏɴᴛᴀᴄᴛ @ITSANIMEN."
        )
        return None
    ...
    caption=Config.VERIFY_TEXT.format(
        mention=message.from_user.mention
    ),
        reply_markup=buttons
    )
...
async def get_shortlink(link, shortener_num):
    ...
    SHORTENERS = {
        1: {
            "api": "e75f29485ddb045e6887d284f762f3daeb84f869",
            "site": "shrinkme.io"
        },
        2: {
            "api": "3e035de14c827502846298486c13bce202fae43a",
            "site": "gplinks.io"
        }
    }
    #settings = await rexbots.get_verification_settings()
     ...
     #   return None
    shortener = SHORTENERS.get(shortener_num)
    if not shortener:
        logger.error(f"Invalid shortener number: {shortener_num}")
        return None
