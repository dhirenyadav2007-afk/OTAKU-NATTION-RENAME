...
async def tutorial(bot, message):
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("• Sᴜᴘᴘᴏʀᴛ •", url="https://t.me/BotifyX_support"), InlineKeyboardButton("•⚡Mᴀɪɴ ʜᴜʙ •", url="https://t.me/BotifyX_Pro_Botz")]
        ])
    )
# ----------------------------------------
# 𝐌𝐀𝐃𝐄 𝐁𝐘 𝐀𝐁𝐇𝐈
...
async def leaderboard_handler(bot: Client, message: Message):
    try:
        ...
        async def generate_leaderboard(filter_type):
            ...
            lines = [f"<b>{filter_title[filter_type]} Tᴏᴘ 10 Rᴇɴᴀᴍᴇʀs</b>\n",
                     "────────────────────────"
                     ]
            ...
            for idx, user in enumerate(users, 1):
                ...
                try:
                    ...
                    name = html.escape(user.get("first_name", "Anonymous"))
                    mention = f"<b>{name}</b>"
            ...
            if user_rank:
                lines.append(f"<b>Yᴏᴜʀ Rᴀɴᴋ:</b> {user_rank} ᴡɪᴛʜ {user_count} ʀᴇɴᴀᴍᴇs\n")

            lines.append(f"Lᴀsᴛ ᴜᴘᴅᴀᴛᴇᴅ: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
            lines.append(f"<i>**Tʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ᴀᴜᴛᴏ-ᴅᴇʟᴇᴛᴇ ɪɴ {Config.LEADERBOARD_DELETE_TIMER} sᴇᴄᴏɴᴅs**</i>")
